...
func AddAccount(ctx context.Context, a models.Account) error {
	...
	q := `INSERT INTO accounts (user_uuid, login, password, uuid) VALUES ($1,$2,$3,$4)` // ON CONFLICT (user_id, order_id, sum_in, sum_out)  DO NOTHING `
...
func AddFile(ctx context.Context, f models.File) error {
	...
	defer cancelfunc()
	q := `INSERT INTO files (user_uuid, name, data, uuid) VALUES ($1,$2,$3,$4)` // ON CONFLICT (user_id, order_id, sum_in, sum_out)  DO NOTHING `
...
func AddCard(ctx context.Context, c models.Card) error {
	...
	ctx, cancelfunc := context.WithTimeout(ctx, 50*time.Second)
	defer cancelfunc()
	q := `INSERT INTO cards (user_uuid, number, month, year, owner, uuid) VALUES ($1,$2,$3,$4,$5)` // ON CONFLICT (user_id, order_id, sum_in, sum_out)  DO NOTHING `
